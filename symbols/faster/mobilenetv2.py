...
class mobilenetv2(Symbol):
    ...
    def get_symbol_rcnn(self, cfg, is_train=True):
        ...
        if is_train:
            ...
            rois, label, bbox_target, bbox_weight = mx.sym.MultiProposalTarget(cls_prob=rpn_cls_prob, bbox_pred=rpn_bbox_pred, im_info=im_info, gt_boxes=gt_boxes, valid_ranges=valid_ranges, crowd_boxes=crowd_boxes, batch_size=cfg.TRAIN.BATCH_IMAGES, feature_stride=cfg.network.RPN_FEAT_STRIDE, scales=cfg.network.ANCHOR_SCALES, name='multi_proposal_target')
            ...
            cls_prob = mx.sym.SoftmaxOutput(name='cls_prob', data=cls_score, label=label, use_ignore=True, ignore_label=-1, grad_scale=grad_scale / (300.0*cfg.TRAIN.BATCH_IMAGES))
            bbox_loss_ = bbox_weight * mx.sym.smooth_l1(name='bbox_loss_', scalar=1.0,
                                                        data=(bbox_pred - bbox_target))
            bbox_loss = mx.sym.MakeLoss(name='bbox_loss', data=bbox_loss_, grad_scale=grad_scale / (188.0*cfg.TRAIN.BATCH_IMAGES))
            rcnn_label = label
