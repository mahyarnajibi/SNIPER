...
class resnet_mx_101_e2e_3k_demo(Symbol):
    ...
    def get_symbol_rcnn(self, cfg, is_train=True):
        ...
        if is_train:
            ...
            #group = mx.sym.Group([rois, cls_prob, bbox_pred, im_ids, subcls_prob, relu1])
            group = mx.sym.Group([rois, cls_prob, bbox_pred, subcls_prob, relu1])
            #group = mx.sym.Group([rpn_cls_score])
        self.sym = group
        return group
