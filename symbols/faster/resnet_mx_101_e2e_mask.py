...
class resnet_mx_101_e2e_mask(Symbol):
    ...
    def get_mask_head(self, feat, num_layers=4, num_classes=80):
        mask_relu_out = feat
        ...
        mask_out = mx.sym.Convolution(
                data=mask_deconv_relu, kernel=(1, 1), pad=(0, 0), num_filter=80*2, name="mask_out")
        return mask_out
    ...
    def get_symbol_rcnn(self, cfg, is_train=True):
        ...
        if is_train:
            ...
            rois, label, bbox_target, bbox_weight, mask_rois, mask_ids = mx.sym.MultiProposalTargetMask(cls_prob=rpn_cls_prob, bbox_pred=rpn_bbox_pred, im_info=im_info,
                                                                               gt_boxes=gt_boxes, valid_ranges=valid_ranges, batch_size=cfg.TRAIN.BATCH_IMAGES, name='multi_proposal_target_mask')
            ...
            ##################################### Mask Branch #######################################
            # Pool positive RoIs
            mask_offset_t = mx.contrib.sym.DeformablePSROIPooling(name='mask_offset_t', data=conv_new_1_relu, rois=mask_rois,
                                                        group_size=1, pooled_size=14, sample_per_part=4,
                                                        no_trans=True, part_size=14, output_dim=256, spatial_scale=0.0625)
            mask_offset = mx.sym.FullyConnected(name='mask_offset', data=mask_offset_t, num_hidden=14 * 14 * 2, lr_mult=0.01)
            mask_offset_reshape = mx.sym.Reshape(data=mask_offset, shape=(-1, 2, 14, 14), name="mask_offset_reshape")
            
            mask_deformable_roi_pool = mx.contrib.sym.DeformablePSROIPooling(name='mask_deformable_roi_pool', data=conv_new_1_relu,
