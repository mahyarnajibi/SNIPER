...
class MNIteratorTest(MNIteratorBase):
    def __init__(self, roidb, config, batch_size=4, threads=8, nGPUs=1, pad_rois_to=400, crop_size=(512, 512)):
        ...
        self.im_worker = im_worker(crop_size=None if not self.crop_size else self.crop_size[0], cfg=config)
    ...
    def _get_batch(self, roidb):
        ...
        max_size = [self.cfg.SCALES[0][0], self.cfg.SCALES[0][1]] if hor_flag else \
            [self.cfg.SCALES[0][1], self.cfg.SCALES[0][0]]
        ...
        for i,p in enumerate(processed_list):
            im_info[i] = [p[0].shape[1], p[0].shape[2], p[1]]
            im_tensor[i] = p[0]
